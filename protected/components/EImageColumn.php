...
class EImageColumn extends CGridColumn
{
	...
	{
		$this->name =  preg_replace('%.+/html%','',$data->filename);

		$this->link = $this->name; 
		
// 		$this->alt = $data->time_stamp;
		$this->alt = preg_replace('%.+/%','',$data->filename);
		
		$image = CHtml::image($this->name,
				$this->alt,
				$this->htmlOptions);

		$htmlOptions = array();
		foreach ($this->linkHtmlOptions as $key=>$value)
		{
			
			if (isset($data->$value))
			{
				$htmlOptions[$key] = $data->$value;
			}
			else
			{
		...
		if($this->link)
			echo CHtml::link($image, $this->link, $htmlOptions);
		else
			echo $image;
	}
}
