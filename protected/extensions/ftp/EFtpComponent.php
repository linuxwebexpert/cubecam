...
class EFtpComponent extends CApplicationComponent
{
    ...
    */
	 ...
	 */
	public function __construct($host=null, $username=null, $password=null, $passive = true, $ssl=false, $port=21, $timeout=90)
	{
		$this->host = $host;
    ...
    public function connect()
    {
        if($this->_connection === null){
            ...
            if($this->passive)
            {
            	$passiveResult = ftp_pasv($this->_connection, true);
            	if(!$passiveResult)
            		throw new CException('FTP Library Error: Cannot Enter Passive mode!');
            }
        }
    }
    /**
    ...
    public function passive($mode=true)
    {
    	if($this->getAction())
    	{
    		$passiveResult = ftp_pasv($this->_connection, $mode);
    		if($passiveResult)
    		{
    			$this->passive = $mode;
    			return true;
